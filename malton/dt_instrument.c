...
static IRType shadowTypeV ( IRType ty ) //348
{
	switch (ty) {
		...
		default: ppIRType(ty);
						 VG_(tool_panic)("dt_translate.c: shadowTypeV");
	}
}
...
static IRExpr* definedOfType ( IRType ty ) { //367
	switch (ty) {
		...
		default:       VG_(tool_panic)("dt_translate.c: definedOfType");
	}
}
...
static inline void stmt( HChar cat, MCEnv* mce, IRStmt* st ) { //385
	...
	// [ UNTITLED ] This is a push/append operation: push `st`
	// into mce->stmts (reallocate a larger space if full)
}
...
static IRAtom* mkUifU ( MCEnv* mce, IRType vty, IRAtom* a1, IRAtom* a2 ) {
	switch (vty) {
		...
		default:
			...
			ppIRType(vty); 
			VG_(printf)("\n");
			VG_(tool_panic)("dt_translate.c:mkUifU");
	}
}
...
static IRAtom* mkPCastTo( MCEnv* mce, IRType dst_ty, IRAtom* vbits )
{
	...
	switch (src_ty) {
		...
		default:
									 ppIRType(src_ty);
									 VG_(tool_panic)("dt_translate.c: mkPCastTo(1)");
	}
	tl_assert(tmp1);
	...
	switch (dst_ty) {
		...
		default:
			ppIRType(dst_ty);
			VG_(tool_panic)("dt_translate.c: mkPCastTo(2)");
	}
}
...
static Bool isAlwaysDefd ( MCEnv* mce, Int offset, Int size ) //1159
{
	...
	for (i = 0; i < mce->layout->n_alwaysDefd; i++) {
		...
		VG_(tool_panic)("dt_translate.c: isAlwaysDefd:partial overlap");
	}
		...
		IRAtom* atom, IRAtom* vatom, IRExpr *guard )
{
	...
	//   if (dt_clo_dt_level == 1)
	//      return;

	if (atom) {
		tl_assert(!vatom);
		tl_assert(isOriginalAtom(mce, atom));
		/* Get the shadow info from atom and store it into vatom */
		vatom = atom2vbits( mce, atom );
	} else {
		tl_assert(vatom);
		tl_assert(isShadowAtom(mce, vatom));
	...
	/* S1: Do taint propagation from vatom to shadow r_offset */
	...
	if( atom && clone && !dt_clo_critical_ins_only ){
		/* S2: Add dirty fn for checking taint info of atom */
		...
		IRAtom* atom3, IRAtom* atom4 )
{
	...
	switch (op) {
		...
		default:
			ppIROp(op);
			VG_(tool_panic)("dt_translate.c:expr2vbits_Qop");
	}
}
		...
		IRAtom* atom1, IRAtom* atom2, IRAtom* atom3 )
{
	...
	switch (op) {
		...
		default:
			ppIROp(op);
			VG_(tool_panic)("dt_translate.c:expr2vbits_Triop");
	}
}
...
do_And_Or:
		...
		default:
										 ppIROp(op);
										 VG_(tool_panic)("dt_translate.c: expr2vbits_Binop");
	}
}
...
IRExpr* expr2vbits_Unop ( MCEnv* mce, IROp op, IRAtom* atom )
{
	...
	switch (op) {
		...
		case Iop_V128to32: // Rewhy this op is not implemented in ARM
			return assignNew('V', mce, Ity_I32, IRExpr_Const( IRConst_U32( 0x0 )));
		...
		default:
			ppIROp(op);
			VG_(tool_panic)("dt_translate: expr2vbits_Unop");
	}
}
		...
		IRAtom* addr, UInt bias, IRAtom* guard )
{
	...
	if (end == Iend_LE) {
		switch (ty) {
			case Ity_V256: helper = &dt_helperc_LOADV256le;
										 hname = "dt_helperc_LOADV256le";
										 ret_via_outparam = True;
										 break;
			case Ity_V128: helper = &dt_helperc_LOADV128le;
										 hname = "dt_helperc_LOADV128le";
										 ret_via_outparam = True;
										 break;
			case Ity_I64:  helper = &dt_helperc_LOADV64le;
										 hname = "dt_helperc_LOADV64le";
										 break;
			case Ity_I32:  helper = &dt_helperc_LOADV32le;
										 hname = "dt_helperc_LOADV32le";
										 break;
			case Ity_I16:  helper = &dt_helperc_LOADV16le;
										 hname = "dt_helperc_LOADV16le";
										 break;
			case Ity_I8:   helper = &dt_helperc_LOADV8;
										 hname = "dt_helperc_LOADV8";
										 break;
			default:       ppIRType(ty);
										 VG_(tool_panic)("dt_translate.c:expr2vbits_Load_WRK(LE)");
		}
	} else {
		switch (ty) {
			case Ity_V256: helper = &dt_helperc_LOADV256be;
										 hname = "dt_helperc_LOADV256be";
										 ret_via_outparam = True;
										 break;
			case Ity_V128: helper = &dt_helperc_LOADV128be;
										 hname = "dt_helperc_LOADV128be";
										 ret_via_outparam = True;
										 break;
			case Ity_I64:  helper = &dt_helperc_LOADV64be;
										 hname = "dt_helperc_LOADV64be";
										 break;
			case Ity_I32:  helper = &dt_helperc_LOADV32be;
										 hname = "dt_helperc_LOADV32be";
										 break;
			case Ity_I16:  helper = &dt_helperc_LOADV16be;
										 hname = "dt_helperc_LOADV16be";
										 break;
			case Ity_I8:   helper = &dt_helperc_LOADV8;
										 hname = "dt_helperc_LOADV8";
										 break;
			default:       ppIRType(ty);
										 VG_(tool_panic)("dt_translate.c::expr2vbits_Load_WRK(BE)");
		}
	}
	...
	IRAtom* vbits_data = mkexpr(datavbits);
	/* Get taint info of the tmp */
	if(addr->tag == Iex_RdTmp && guard==NULL && ty == Ity_I16) { // Rewhy
		IROp opTO; // Currently for only 32bit
		IRAtom* vbits_tmp
			= expr2vbits( mce, addr );
		IRAtom* vbits_data_tmp;
		switch (ty) {
			case Ity_I32:
				return assignNew('V', mce, Ity_I32, binop(Iop_Or32, vbits_data, vbits_tmp));
			case Ity_I8:
				opTO = Iop_32to8;
				vbits_data_tmp = assignNew('V', mce, Ity_I32, unop(Iop_8Uto32, vbits_data));
				break;
			case Ity_I16:
				opTO = Iop_32to16;
				vbits_data_tmp = assignNew('V', mce, Ity_I32, unop(Iop_16Uto32, vbits_data));
				break;
			case Ity_I64:
				opTO = Iop_32Uto64;
				vbits_data_tmp = assignNew('V', mce, Ity_I32, unop(Iop_64to32, vbits_data));
				break;
			default:
				VG_(tool_panic)("loadtype");
		}
		IRAtom* vbits_res1 = assignNew('V', mce, Ity_I32, binop(Iop_Or32, vbits_tmp, vbits_data_tmp));
		return assignNew('V', mce, ty, unop(opTO, vbits_res1));
	} else {
		return vbits_data;
	}
}
...
{
	switch (e->tag) {
		case Iex_Get: /* Get the taint info of register (offset) */
		...
		case Iex_RdTmp:/* Get the value of the shadow tmp */
			return IRExpr_RdTmp( findShadowTmpV(mce, e->Iex.RdTmp.tmp) );

		case Iex_Const:/* Defile taint info 0 for the const with conresponding type */
			return definedOfType(shadowTypeV(typeOfIRExpr(mce->sb->tyenv, e)));

		case Iex_Qop:	 /* A quaternary operation */
		...
		case Iex_Binop: /* A binary operation, get both taint values for this two args */
		...
		default:
			...
			ppIRExpr(e);
			VG_(printf)("\n");
			VG_(tool_panic)("dt_translate.c: expr2vbits: Unhandled expression");
	}
}
...
IRExpr* atom2vbits ( MCEnv* mce, IRAtom* atom )
{
	...
	switch (atom->tag) {
		...
		default:
			...
			ppIRExpr(atom);
			VG_(printf)("\n");
			VG_(tool_panic)("dt_translate.c: atom2vbits: Unhandled expression");
			return NULL;
	}
}

// t<tmp> = <data>: include checks for tainted RdTmp's
	static
void do_shadow_WRTMP ( MCEnv* mce, IRStmt *clone, IRTemp tmp, IRExpr* expr )
{
	IRDirty* di2;

	/* Add original stmt to BB */
	stmt( 'C', mce, IRStmt_WrTmp( tmp, expr ) );
	/* Add taint propagating stmt (from expr to tmp) to BB */
		...
		IRAtom* guard )
{
	...
	/*   if (dt_clo_dt_level == 1) {
			 ...
			 default:       VG_(tool_panic)("dt_translate.c:do_shadow_Store(LE)");
			 }
	...
	if (end == Iend_LE) {
		switch (ty) {
			...
			case Ity_I64: helper = &dt_helperc_STOREV64le;
										hname = "dt_helperc_STOREV64le";
										break;
			case Ity_I32: helper = &dt_helperc_STOREV32le;
										hname = "dt_helperc_STOREV32le";
										break;
			case Ity_I16: helper = &dt_helperc_STOREV16le;
										hname = "dt_helperc_STOREV16le";
										break;
			case Ity_I8:  helper = &dt_helperc_STOREV8;
										hname = "dt_helperc_STOREV8";
										break;
			default:      VG_(tool_panic)("dt_translate.c:do_shadow_Store(LE)");
		}
	} else {
		switch (ty) {
			case Ity_V128: /* we'll use the helper twice */
			case Ity_I64: helper = &dt_helperc_STOREV64be;
										hname = "dt_helperc_STOREV64be";
										break;
			case Ity_I32: helper = &dt_helperc_STOREV32be;
										hname = "dt_helperc_STOREV32be";
										break;
			case Ity_I16: helper = &dt_helperc_STOREV16be;
										hname = "dt_helperc_STOREV16be";
										break;
			case Ity_I8:  helper = &dt_helperc_STOREV8;
										hname = "dt_helperc_STOREV8";
										break;
			default:      VG_(tool_panic)("dt_translate.c:do_shadow_Store(BE)");
		}
	}
...
static IRType szToITy ( Int n )
{
	switch (n) {
		...
		default: VG_(tool_panic)("dt_translate.c: szToITy");
	}
}
	...
	for (i = 0; i < d->nFxState; i++) {
		...
		for (k = 0; k < 1 + d->fxState[i].nRepeats; k++) {
			...
			if (isAlwaysDefd(mce, gOff, gSz)) {
				if (0)
					VG_(printf)("dt_translate: Dirty gst: ignored off %d, sz %d\n",
							gOff, gSz);
				continue;
			}
...
//           VG_(fnptr_to_fnentry)( &dt_helperc_make_stack_uninit ),
...
static void do_shadow_CAS_single ( MCEnv* mce, IRCAS* cas )
{
	...
	//Int    elemSzB;
	IRType elemTy;
	//   Bool   otrak = dt_clo_dt_level >= 3; /* a shorthand */
...
static void do_shadow_CAS_double ( MCEnv* mce, IRCAS* cas )
{
	...
	IRType elemTy;
	//   Bool   otrak = dt_clo_tnt_level >= 3; /* a shorthand */
...
static void do_shadow_LoadG ( MCEnv* mce, IRLoadG* lg )
{
	...
	/* Get taint info of the source addr */
	IRAtom* vbits_alt
...
unhandled:
										ppIRStmt(st);
										VG_(tool_panic)("dt_translate.c: checkForBogusLiterals");
	}
}
...
passed from dt_stm2_instrument to its subroutines to 
...
Int extract_IRConst( IRConst* con ){
	switch(con->tag){
		...
		default:
			ppIRConst(con);
			VG_(tool_panic)("dt_translate.c: convert_IRConst");
	}
}
	...
	if(tyH == Ity_I32){
		switch( ty ){
			...
			case Ity_V128: // Comment by Rewhy
				...
				return assignNew('C', mce, Ity_I32, IRExpr_Const( IRConst_U32( 0x0 )));
			default:
				ppIRType(ty);
...
IRDirty* create_dirty_PUT( MCEnv* mce, IRStmt *clone, Int offset, IRExpr* data ){
	...
	if(mce->hWordTy == Ity_I32 && data->tag == Iex_RdTmp){
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_PUT: Unknown type");
...
IRDirty* create_dirty_PUTI( MCEnv* mce, IRRegArray* descr, IRExpr* ix, Int bias, IRExpr* data ){
	...
	if(mce->hWordTy == Ity_I32){
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_PUTI: Unknown type");
...
IRDirty* create_dirty_STORE( MCEnv* mce, IRStmt *clone,
	...
	if(mce->hWordTy == Ity_I32){
		if ( addr->tag == Iex_RdTmp && data->tag == Iex_RdTmp ) {
			...
			VG_(tool_panic)("dt_translate.c: create_dirty_STORE: unk 32-bit cfg");
		}
	}else if(mce->hWordTy == Ity_I64){
		if ( addr->tag == Iex_RdTmp && data->tag == Iex_RdTmp ) {
			fn    = &dt_h64_store_tt;
			...
			ppIRExpr(addr);
			ppIRExpr(data);
			VG_(tool_panic)("dt_translate.c: create_dirty_STORE: unk 64-bit cfg");
		}
	}else
		VG_(tool_panic)("dt_translate.c: create_dirty_STORE: Unknown platform");
...
IRDirty* create_dirty_EXIT( MCEnv* mce, IRStmt *clone, IRExpr* guard, IRJumpKind jk, IRConst* dst ){
	...
	if(mce->hWordTy == Ity_I32 && guard->tag == Iex_Const){
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_EXIT: Unknown platform");
...
IRDirty* create_dirty_NEXT( MCEnv* mce, IRExpr* next ){
	...
	if(mce->hWordTy == Ity_I32 && next->tag == Iex_Const){
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_NEXT: Unknown platform");
...
IRDirty *create_dirty_WRTMP_const( MCEnv *mce, IRStmt *clone, IRTemp tmp )
{
	...
	if ( mce->hWordTy == Ity_I32 ) {
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_WRTMP_const: Unknown platform");

	return unsafeIRDirty_0_N ( nargs/*regparms*/, nm, VG_(fnptr_to_fnentry)( fn ), args );
}

#if 0
IRDiry* create_dirty_Const( MCEnv* mce, IRStmt *clone, IRTemp tmp, IRConst* con ){
	Int			nargs = 3;
	...
	if(mce->hWordTy == Ity_I32) {
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_Const: Unknown platform");
	}
...
IRDirty* create_dirty_GET( MCEnv* mce, IRStmt *clone, IRTemp tmp, Int offset, IRType ty ){
	...
	if(mce->hWordTy == Ity_I32){
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_GET: Unknown platform");
	}
...
IRDirty* create_dirty_RDTMP( MCEnv* mce, IRStmt *clone, IRTemp tmp, IRTemp data ){
	...
	if(mce->hWordTy == Ity_I32){
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_RDTMP: Unknown platform");
...
IRDirty* create_dirty_BINOP( MCEnv* mce, IRStmt *clone,
	...
	if ( mce->hWordTy == Ity_I32 ){
		if ( arg1->tag == Iex_RdTmp && arg2->tag == Iex_Const ) {
			...
			VG_(tool_panic)("dt_translate.c: create_dirty_BINOP");
		}
	}else if( mce->hWordTy == Ity_I64 ){
		if ( arg1->tag == Iex_RdTmp && arg2->tag == Iex_Const ) {
			fn = &dt_h64_binop_tc;
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_BINOP: Unknown platform");
...
IRDirty* create_dirty_UNOP( MCEnv* mce, IRStmt *clone, IRTemp tmp, IROp op, IRExpr* arg ){
	...
	if ( mce->hWordTy == Ity_I32 && arg->tag == Iex_Const ) {
		...
		VG_(tool_panic)("dt_translate.c: create_dirty_UNOP: Unknown platform");
	}
...
IRDirty* create_dirty_CCALL( MCEnv* mce, IRStmt *clone, IRTemp tmp,
	...
	if ( mce->hWordTy == Ity_I32 ) {
		if ( VG_(strcmp)(cee->name, "x86g_calculate_condition") == 0 ) {
			fn    = &dt_h32_x86g_calculate_condition;
			nm    = "dt_h32_x86g_calculate_condition";
		} else {
			fn    = &dt_h32_ccall;
			nm    = "dt_h32_ccall";
		}
		...
		if ( VG_(strcmp)(cee->name, "amd64g_calculate_condition") == 0 ) {
			fn    = &dt_h64_amd64g_calculate_condition;
			nm    = "dt_h64_amd64g_calculate_condition";
		} else {
			fn    = &dt_h64_ccall;
			nm    = "dt_h64_ccall";
		}
...
IRDirty* create_dirty_ITE( MCEnv* mce, IRStmt *clone, IRTemp tmp,
	...
	if(mce->hWordTy == Ity_I32){
		if ( iftrue->tag == Iex_RdTmp && iffalse->tag == Iex_Const ) {
			...
			VG_(tool_panic)("dt_translate.c: create_dirty_ITE: Unknown 32-bit mode");
	}else if(mce->hWordTy == Ity_I64){
		if ( iftrue->tag == Iex_RdTmp && iffalse->tag == Iex_Const ) {
			fn    = &dt_h64_ite_tc;
			nm    = "dt_h64_ite_tc";
			...
			VG_(tool_panic)("dt_translate.c: create_dirty_ITE: Unknown 64-bit mode");
	}else
		VG_(tool_panic)("dt_translate.c: create_dirty_ITE: Unknown platform");
...
//static FilterList filterList[256];
struct LibList *llist = NULL;

static Int filterNum = 0;
static struct FilterList *fl = NULL;

static DebugInfo *di_art = NULL;
...
Bool addFilterFun(const HChar* soname, const HChar* fnname) {
	...
	/* add new library to filter list */
	if( !tll ) {
	...
	if( fnname ) {
		while( tfl ) {
			if(VG_(strcmp)(fnname, tfl->name) == 0) {
				DT_LOGI("add %s in %s already existed\n", fnname, soname);
				return False;
			}
			tfl = tfl->next;
		}
...
void addFilterList( Addr avma, SizeT size ) {
	struct FilterList *tfl;
	tfl = (struct FilterList*)VG_(malloc)("addFilterFun.3", sizeof( struct FilterList ));
	tfl->text_avma = avma;
	tfl->text_size = size;
	tfl->text_end = avma + size;
	tfl->next = fl;
	fl = tfl;
	filterNum++;
	DT_LOGI("filter sec 0x%x - 0x%x added \n", 
			tfl->text_avma,
			tfl->text_end);
}
void initFilterlist() {
	...
	while(di) {
		...
		if(VG_(DebugInfo_is_oat)(di)) {
			...
			if( tll )
			{
				DT_LOGI("Add library %s to list\n", soname);
				tfl = tll->flist;
				if( tfl ) {
					while( tfl ) {
						tl_assert(tfl->name);
						DT_LOGI("Add function %s to list\n", tfl->name); // Rewhy no the following funciton
...
Bool is_instrument_needed( VgCallbackClosure* closure ) {
	struct FilterList* tfl = fl, *ltl = NULL;
	Addr a = closure->nraddr;
	while ( tfl ) {
		if( tfl->text_avma <= a && a < tfl->text_end )
		{
			if (ltl) {
				ltl->next = tfl->next;
				tfl->next = fl;
...
static inline void addStmt( MCEnv *mce, IRStmt *st) {
	addStmtToIRSB(mce->sb, st);
}
...
INLINE void mth_stack_print(ThreadId tid) {
	...
	if(tid < TG_N_THREADS) {
		...
		for (Int i = ms->size; i > 0; i--) {
			mn = (MthNode*)ms->mth[i-1];
			VG_(printf)("%03d %05d 0x%08x %s %s() %s\n",
					i, mn->mthKey, (Addr)ms->addr[i-1],
					mn->clazz, mn->method, mn->shorty);
		}
	}
}
...
static void invoke_superblock(MthList* mList, VexGuestLayout *layout)
{
	// UN_PRINT_FUNC();
	ThreadId tid			= VG_(get_running_tid)();
	ThreadState *tst	= VG_(get_ThreadState) ( tid );
	VexGuestArchState *arch_state = &tst->arch.vex;  // [ UNTITLED ] Looks like a bundled guest CPU state
...
#endif
	struct ArtMethodPlus *pAMth = (struct ArtMethodPlus *)r0;  // [ UNTITLED ] What is in `r0`?
	// [ UNTITLED ] Somehow, `r0` points to a `ArtMethodPlus` structure, which probably
	// containts a bunch of compiled method (current method?) meta information
	MthNode *mNode = NULL;//query_method_node(pc&0xfffffffe, pAMth->dex_method_index_);
	// VG_(printf)("DBG: %s:%d mList=0x%08x r0=0x%08x pc=0x%08x\n", __func__, __LINE__, (Addr)mList, r0, pc);
	Int i;
	for(i = 0; i < mList->num; i++) {
		// [ UNTITELD ] Traverse the `mList` (method list?)
		mNode = (MthNode *)mList->mthNodes[i];
		if(mNode->mthKey == pAMth->dex_method_index_)
			break;  // [ UNTITLED ] The corresponding `MthNode` was found
	}
	// VG_(printf)("DBG: %s:%d\n", __func__, __LINE__);
...
#if 0
static void check_return_value(HChar type,  VexGuestLayout *layout) {
	ThreadId tid = VG_(get_running_tid)();
	ThreadState *tst	= VG_(get_ThreadState) ( tid );
	VexGuestArchState *arch_state = &tst->arch.vex;
	StringObject *strObj = NULL;
	UShort *value = NULL; Int i = 0;
	DT_LOGI("Check return value %c\n", type);
	switch(type) {
		case 'L':
#if defined(VGPV_arm_linux_android)
			strObj = (StringObject*)arch_state->guest_R0;
			make_string_tainted(strObj);
#endif
			break;
		default:
			break;
...
static void return_superblock(Addr a,  VexGuestLayout *layout)
{
	// UN_PRINT_FUNC();
	ThreadId tid = VG_(get_running_tid)();
	ThreadState *tst	= VG_(get_ThreadState) ( tid );
	...
	if(mth_top_stack(tid, &addr, &mNode)) {
		if((addr&0xfffffffe) == (a&0xfffffffe)) {
			/*ART_RETURN("%d %05d pc=0x%08x %s %s() %s source=%d sp=0x%08x\n", tid, mNode->mthKey, 
				...
				mNode->type, sp);*/
			isStatic = (mNode->accessFlags & ACC_STATIC) ? True : False;  // [ UNTITLED ] Static method?
			Bool isSource = mNode->type & TYPE_SOURCE;
			HChar tag[8];
			VG_(sprintf)(tag, " tag=%02x", mNode->tag);
			ART_RETURN("%d %05d %s %s() %s isSource=%s%s\n", 
					tid, mNode->mthKey, mNode->clazz, mNode->method, 
					mNode->shorty,
					isSource ? "True" : "Flase",
					isSource ? tag: "");
			mth_pop_stack(tid);
			if(trace_obj_taint) { // Do object taint (for ART method)
				if(isSource) {
					// [ UNTITLED ] If the method to be invoked is marked as
					// a taint source, then the returned value must be marked
					// tainted
					do_taint_source(mNode, tid);
					// VG_(printf)("[ UNTITLED ] [ RETURN ] s0=%08X s1=%08X s2=%08X s3=%08X\n",
					// 		((UInt *) s)[0], ((UInt *) s)[1], ((UInt *) s)[2], ((UInt *) s)[3]);
				} else {
					// [ UNTITLED ] If the method to be invoked is not a predefined
					// taint source, then do normal checks (?)
...
#define TRACE_ART_METHOD 1

IRSB* dt_stm2_instrument( VgCallbackClosure* closure,
		IRSB* sbIn,
		...
		IRType	gWordTy, IRType	hWordTy )
{
	...
	if ( dt_clo_taint_begin == False ) {
		return sbIn;  // [ UNTITLED ] If the tainting has not started yet, no need to do tainting propagation
	}

#ifdef DT_WHITE_LIST
	if (LIKELY(is_instrument_needed( closure ) == False))
#else
		if (UNLIKELY(is_instrument_needed( closure )))
#endif
		{
			return sbIn;
		}
	// For complain if tainted
	IRDirty* di2;
	...
	// UN_PRINT_DEBUG("[ UNTITLED ] [ DEBUG ] Instrumenting...\n");  // Alright, we are really doing some instrumentation

	/* Set up SB */
	sbOut = deepCopyIRSBExceptStmts(sbIn);

	/* Set up the running environment. Both .sb and .tmpMap are modified
	 * as we go along. Note that tmps are added to both .sb->tyenv and 
	 * .tmpmap together, so the valid index-set for those two arrays should
	 * alwarys be identical.
	 * */
	VG_(memset)(&mce, 0, sizeof(mce));
	mce.sb					= sbOut;
	mce.trace				= verboze;
	mce.layout			= guestlayout;
	mce.hWordTy			= hWordTy;
	...
	mce.tmpMap = VG_(newXA)( VG_(malloc), "mc.dt_stm2_instrument.1", VG_(free),
			sizeof(TempMapEnt));

	i = 0;
	while(i < sbIn->tyenv->types_used ){
		TempMapEnt ent;
		ent.kind		= Orig;
		ent.shadowV = IRTemp_INVALID;
	...
	tl_assert( VG_(sizeXA)( mce.tmpMap ) == sbIn->tyenv->types_used );
	...
	i = 0;
	while(i < sbIn->stmts_used) {  // [ UNTITLED ] This should be a for-loop
		st = sbIn->stmts[i];  // Iterate statements in `sbIn` (input super block?)
	...
	/* Copy IR preamble until the first IMark */
	// [ UNTITLED ] What is IR preamble?
	tl_assert(mce.sb == sbOut);
	tl_assert(mce.sb != sbIn);

	i = 0;
	while(i < sbIn->stmts_used && sbIn->stmts[i]->tag != Ist_IMark) {  // [ UNTITLED ] IMark: external/valgrind/VEX/pub/libvex_ir.h:2686
		// Iterate the statements in `sbIn`, and break the iteration when encountering
		// a `IMark`
		st = sbIn->stmts[i];
		...
		stmt( 'C', &mce, st );  // [ UNTITLED ] Push `st` into `mce->stmts` (why?)

		i++;
	}

	/* Scan the preamble and look for assignments to temporaries */
	for(j = 0; j < i; j++) {
		if(sbIn->stmts[j]->tag == Ist_WrTmp) {
			// If the j-th statement is a "temporary assignment"

			/* Get the original Tmp */
			IRTemp tmp_o = sbIn->stmts[j]->Ist.WrTmp.tmp;  // Left-hand side of the assignment
			/* Create the shadow Tmp for original Tmp */
			IRTemp tmp_v = findShadowTmpV(&mce, tmp_o);
			/* Get the type of the shadow Tmp */
			IRType ty_v  = typeOfIRTemp(mce.sb->tyenv, tmp_v);
			/* Note that: V128toV32 is not supported by Coregrind on aarch64 */
			/* Add the shadow Tmp to the output BB */
			assign( 'V', &mce, tmp_v, definedOfType( ty_v ));  // [ UNTITLED ] ???
#if	DBT_TAINT_INFO
			DT_LOGI("Create shadow tmp(s) for preamble temp[%d] ty ", j);
			ppIRType( ty_v );
			DT_LOGI("\n");
#endif
		}
	...
	if(trace_art_method) {
		...
		if(is_framework_bb(vge->base[0])) {
			// UN_PRINT_DEBUG("[ UNTITLED ] [ DEBUG ] Inserting `invode_superblock`\n");
			...
			if(mList){
				di = unsafeIRDirty_0_N( 
					...
					mkIRExprVec_2( // NULL-terminated IRExpr vector: [ mList, guestLayout, NULL ]
						mkIRExpr_HWord((Addr)mList),
						mkIRExpr_HWord((Addr)guestlayout)
					)
				);
				stmt('C', &mce, IRStmt_Dirty(di) );  // Insert the function
				isEntry = True;
			}
		}
		if(isEntry == False) {
			di = unsafeIRDirty_0_N( 
					...
					mkIRExprVec_2(mkIRExpr_HWord(vge->base[0]&~0x1),
						mkIRExpr_HWord((Addr)guestlayout)));
			stmt('C', &mce, IRStmt_Dirty(di) );
			//VG_(printf)("0x%08x 0x%08x %d\n", vge->base[0], (Addr)guestlayout, guestlayout->total_sizeB);
		}
	}
	
	if(trace_ins_taint == False) { // Run without instruction level taint propagation
		for (; i < sbIn->stmts_used; i++) {
			st = sbIn->stmts[i];
			stmt('C', &mce, st);  // [ UNTITLED ] Again??? (3 times?)
		}
		return sbOut;
	}
	/* Instrument each BB instructions */
	for (/* use existing i */; i < sbIn->stmts_used; i++) {
		st = sbIn->stmts[i];
		...
		//addStmt( &mce, st );
		switch (st->tag) {
			case Ist_WrTmp:			// t<tmp> = <data>
				do_shadow_WRTMP( &mce, 
						stclone,
						st->Ist.WrTmp.tmp,
						st->Ist.WrTmp.data);
				break;
			case Ist_Put:				// PUT(<offset>) = <data>
				do_shadow_PUT( &mce, 
						stclone,
						st->Ist.Put.offset,
						st->Ist.Put.data,
						NULL /* shadow atom */,
						NULL /* guard */);
				/* S3: Add the original stmt to BB in the End */
				break;
			case Ist_PutI:			// PUTI<descr>[<ix>,<bias>] = <data>
				do_shadow_PUTI( &mce, st->Ist.PutI.details);
				break;
			case Ist_Store:			// ST<end>(<addr>) = <data>
				do_shadow_Store( &mce, stclone,
						st->Ist.Store.end,
						st->Ist.Store.addr,
						0 /* addr bias */,
						st->Ist.Store.data,
			...
			case Ist_Exit: // Conditional jumps, if(t<guard>) goto {Boring} <addr>:I32
				di2 = create_dirty_EXIT( &mce, stclone, st->Ist.Exit.guard,
						st->Ist.Exit.jk, st->Ist.Exit.dst );
				complainIfTainted( &mce, st->Ist.Exit.guard, di2 );
				break;
			case Ist_IMark:			// IMark(<addr>, <len>, <delta>))
				break;
			case Ist_NoOp:
			case Ist_MBE:
				break;
			...
			case Ist_Dirty:
				...
				break;
			case Ist_AbiHint:		// AbiHint(<base>, <len>, <nia>)
				break;
			case Ist_CAS:				// t<tmp> = CAS<end>(<addr> :: <expected> -> <new>) 
			...
			default:
				...
				ppIRStmt(st);
				VG_(printf)("\n");
				VG_(tool_panic)("dt_translate.c: dt_stm2_instrument: unhandled IRStmt");
		} /* switch (st->tag) */
	...
	di2 = create_dirty_NEXT( &mce, sbIn->next );
	if ( di2 ) complainIfTainted( &mce, sbIn->next, di2 );

	tl_assert( VG_(sizeXA)( mce.tmpMap ) == mce.sb->tyenv->types_used );
	VG_(deleteXA)( mce.tmpMap );

	tl_assert(mce.sb == sbOut);
#if 0 // print output BB
	if( dt_clo_taint_begin ) {
		VG_(printf)("Output: \n");
		ppIRSB(sbOut);
	}
