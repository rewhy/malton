...
inline static void toLowerCase(HChar *dst, HChar *src) {
	while(*src) {
		if('A' <= *src && *src <= 'Z') *dst = *src + 'a' - 'A';
		else *dst = *src;
		dst++;
		src++;
	}
...
static int KMP(HChar *target, HChar *pattern) {
	int targetLength = VG_(strlen)(target);
	int patternLength = VG_(strlen)(pattern);
	if(targetLength == 0 && patternLength == 0) return 0;
	if(targetLength == 0 || patternLength == 0 || targetLength < patternLength) return -1;

	// Case insensitive
	HChar _target[targetLength + 1];
	HChar _pattern[patternLength + 1];
	toLowerCase(_target, target);
	toLowerCase(_pattern, pattern);

	int next[patternLength + 1];
	for(int i = 0; i < patternLength + 1; i++) next[i] = 0;
	for(int i = 1; i < patternLength; i++) {
		int j = next[i + 1];
		while(j > 0 && _pattern[j] != _pattern[i]) j = next[j];
		if(j > 0 || _pattern[j] == _pattern[i]) next[i + 1] = j + 1;
	}

	for(int i = 0, j =0; i < targetLength; i++) {
		if(_target[i] == _pattern[j]) {
			if(++j == patternLength) return i - j + 1;
		} else if(j > 0) {
			j = next[j];
			i--;
		}
...
#define TAINT_SOURCES_LENGTH 7
static const TaintSourceDesc TAINT_SOURCES[TAINT_SOURCES_LENGTH] = {
	{ .clazz = "TelephonyManager", .method = "getDeviceId", .tag = TAG_DEVICE },
	{ .clazz = "Landroid/location/Location;", .method = NULL, .tag = TAG_LOCATION },
	{ .clazz = "Lorg/apache/http/message/ParserCursor;", .method = NULL, .tag = TAG_NONE },
	{ .clazz = "Cursor;", .method = "get", .tag = TAG_DATABASE },
	{ .clazz = "Landroid/hardware/camera", .method = NULL, .tag = TAG_MEDIA },
	// { .clazz = "EncodedValueReader", .method = NULL, .tag = TAG_NONE },
	// { .clazz = "Reader;", .method = "read", .tag = TAG_FILESYSTEM },
	// { .clazz = "InputStream;", .method = "read", .tag = TAG_FILESYSTEM },
	{ .clazz = "Landroid/provider/Settings$Secure;", .method = "get", .tag = TAG_DEVICE },
	{ .clazz = "Landroid/app/usage/NetworkStats$Bucket;", .method = "get", .tag = TAG_USAGE }
};

static UChar getTaintTag(HChar *clazz, HChar* method, HChar* shorty) {
	UNUSED(shorty);
	// Traverse the taint source list and find a match
	const TaintSourceDesc *source = NULL;
	for(int i = 0; i < TAINT_SOURCES_LENGTH; i++) {
		source = &TAINT_SOURCES[i];
		if(source->clazz) {  // Class name pattern is specified
			if(KMP(clazz, source->clazz) >= 0) {  // Class name matched
				if(source->method) {  // Method name pattern is also specified
					if(KMP(method, source->method) >= 0) return source->tag;  // Method name also matched
				} else return source->tag;  // Method name pattern is `any`
			}  // Else: class name pattern not matched
		} else {  // Class name pattern is not specified
			if(source->method) {  // However, method name pattern is specified
				if(KMP(method, source->method) >= 0) return source->tag;  // Method name matched
			} else return source->tag;  // [IMPOSSIBLE] Method name pattern is `any`
		}
	}
	return TAG_NONE;
}
...
VgHashTable* dt_framework_mth_list = NULL;
...
static void* add_method(HChar *clazz, HChar* mth, HChar* shorty, Addr codeAddr, SizeT codeSize, Int index, Int accessFlags)
{
	...
	if(mth_node) {
		...
		mth_node->tag					= getTaintTag(clazz, mth, shorty);
		if(mth_node->tag) {
		...
		if(mth_list == NULL) {
			...
			VG_(HT_add_node)(dt_framework_mth_list, mth_list);
		}
...
#if 0
		VG_(printf)("1 Add Method(%d): 0x%08x %s %s() %s source=%s\n", index, codeAddr, mth_node->clazz, 
				mth_node->method, mth_node->shorty, mth_node->type&TYPE_SOURCE ? "True" : "False" );
#endif
		return mth_node;
	}
...
MthList* query_method_list(Addr codeAddr) {
	if(dt_framework_mth_list == NULL)
		return NULL;
	MthList *mth_list = VG_(HT_lookup)( dt_framework_mth_list, codeAddr);
	if(mth_list)
		return mth_list;
	return NULL;
}
...
Bool query_method(Addr codeAddr, HChar **clazz, HChar **mth, HChar **shorty, Int *accFlags)
{
	...
	MthNode *mth_node = VG_(HT_lookup)( dt_framework_mth_list, codeAddr);
	if(mth_node) {
		if(clazz)
...
{
	MthNode *mth_node = VG_(HT_remove)( dt_framework_mth_list, codeAddr);
	if( NULL == mth_node )
		return;
	VG_(free)( mth_node );
	mth_node = NULL;
...
Bool oatDexFileParse(Addr oatdata, 
	...
	if(dt_framework_mth_list == NULL) {
		dt_framework_mth_list = VG_(HT_construct)( "dt_framework_mth_list" );
	}
	...
	for(j = 0; j < dh->classDefsSize; j++) {
		...
		oatDexClassParse(oatdata, dexBuf, offset, oat_class_header, pDex);
	}
	OAT_LOGI("Finished.\n");
	return True;
}
