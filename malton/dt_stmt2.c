...
static char *dt_smt2_concat_32( char *buf, ULong addr, UInt c, UInt v, UInt t )
{
   char tmp[1024];
   ...
   if ( a2t32(t,c+1) )
       VG_(sprintf)(tmp, "(concat a%llx %s)", addr, dt_smt2_concat_32(buf, addr+1, c-1, v, t) );
   else
       VG_(sprintf)(tmp, "(concat #x%02x %s)", a2v32(v,c+1), dt_smt2_concat_32(buf, addr+1, c-1, v, t) );
   VG_(strcpy)(buf, tmp);
   return buf;
}

static char *dt_smt2_concat_64( char *buf, ULong addr, UInt c, ULong v, ULong t )
{
   char tmp[1024];
   ...
   if ( a2t64(t,c+1) )
       VG_(sprintf)(tmp, "(concat a%llx %s)", addr, dt_smt2_concat_64(buf, addr+1, c-1, v, t) );
   else
       VG_(sprintf)(tmp, "(concat #x%02x %s)", a2v64(v,c+1), dt_smt2_concat_64(buf, addr+1, c-1, v, t) );
   VG_(strcpy)(buf, tmp);
   return buf;
}

static char *dt_smt2_concat_indexed( char *buf, UInt t, UInt c, UInt max )
{
   char tmp[1024];
   ...
   VG_(sprintf)(tmp, "(concat t%d_%d_%d %s)", t, _ti(t), max-c, dt_smt2_concat_indexed(buf, t, c-1, max) );
   VG_(strcpy)(buf, tmp);
   return buf;
}

// if tmp GOTO ...
void dt_smt2_exit ( IRStmt *clone )
{
...
static void dt_smt2_loadstore_atmp ( UInt atmp, ULong address )
{
   // Save current assertions
   VG_(printf)("(push)\n");
   // Invert branch
...
static void dt_smt2_load_ltmp_32 ( UInt ltmp, UInt ty, ULong address, UInt value, UInt taint )
{
   char buf[1024];
   ...
   if ( SMT2_ty[ty] == 128 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_32(buf, address, 14, value, taint) );
   } else if ( SMT2_ty[ty] == 64 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_32(buf, address, 6, value, taint) );
   } else if ( SMT2_ty[ty] == 32 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_32(buf, address, 2, value, taint) );
   } else if ( SMT2_ty[ty] == 16 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_32(buf, address, 0, value, taint) );
...
static void dt_smt2_load_ltmp_64 ( UInt ltmp, UInt ty, ULong address, ULong value, ULong taint )
{
   char buf[1024];
   ...
   if ( SMT2_ty[ty] == 128 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_64(buf, address, 14, value, taint) );
   } else if ( SMT2_ty[ty] == 64 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_64(buf, address, 6, value, taint) );
   } else if ( SMT2_ty[ty] == 32 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_64(buf, address, 2, value, taint) );
   } else if ( SMT2_ty[ty] == 16 )
   {
      VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_64(buf, address, 0, value, taint) );
   } else if ( SMT2_ty[ty] == 8 )
   {
      VG_(printf)("(assert (= t%d_%d a%llx))\n", ltmp, _ti(ltmp), address );
   } else {
...
// ltmp = LOAD <ty> const 
void dt_smt2_load32_c ( IRStmt *clone, UInt value, UInt taint )
{
   ...
   if ( is_tainted(ltmp) )
      dt_smt2_load_ltmp_32 ( ltmp, ty, address, value, taint );
...
// ltmp = LOAD <ty> const 
void dt_smt2_load64_c ( IRStmt *clone, ULong value, ULong taint )
{
   ...
   if ( is_tainted(ltmp) )
      dt_smt2_load_ltmp_64 ( ltmp, ty, address, value, taint );
...
// ltmp = LOAD <ty> atmp
void dt_smt2_load32_t ( IRStmt *clone, UInt value, UInt taint )
{
   ...
   if ( is_tainted(atmp) )
      dt_smt2_loadstore_atmp ( atmp, address );
...
// ltmp = LOAD <ty> atmp
void dt_smt2_load64_t ( IRStmt *clone, ULong value, ULong taint )
{
...
static void dt_smt2_store_dtmp( UInt dtmp, ULong address )
{
   tl_assert( tt[dtmp] );
...
// STORE const = dtmp
void dt_smt2_store_ct ( IRStmt *clone )
{
   ...
   if ( is_tainted(dtmp) )
      dt_smt2_store_dtmp ( dtmp, address );
}

// STORE atmp = const
void dt_smt2_store_tc ( IRStmt *clone )
{
...
// STORE atmp = dtmp
void dt_smt2_store_tt ( IRStmt *clone )
{
...
#define smt2_getmsbsMxN(ty) \
      { \
         ...
         VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_indexed(buf, ltmp, ty-2, ty-2) ); \
         tt[ltmp] = ty; \
      }
...
// ltmp = <op> rtmp
void dt_smt2_unop_t ( IRStmt *clone )
{
...
#define smt2_binop_tc_cmpMxN(m, n, ty, zeros, op) \
      { \
         ...
         VG_(printf)("(assert (= t%d_%d %s))\n", ltmp, _ti(ltmp), dt_smt2_concat_indexed(buf, ltmp, n-2, n-2) ); \
         tt[ltmp] = ty; \
      }
...
static void dt_smt2_binop_tc_common ( UInt op, UInt ltmp, UInt rtmp, ULong c ) {
...
static void dt_smt2_binop_ct_common ( UInt op, UInt ltmp, UInt rtmp, ULong c ) {
...
// ltmp = <op> rtmp1 c
void dt_smt2_binop_tc ( IRStmt *clone )
{
   ...
   dt_smt2_binop_tc_common ( op, ltmp, rtmp, c );
}


// ltmp = <op> c rtmp2
void dt_smt2_binop_ct ( IRStmt *clone )
{
   ...
   dt_smt2_binop_ct_common ( op, ltmp, rtmp, c );
}


// ltmp = <op> rtmp1 rtmp2, rtmp1 is tainted, similar to binop_tc
static void dt_smt2_binop_tt_10 ( IRStmt *clone )
{
   UInt ltmp    = clone->Ist.WrTmp.tmp;
   UInt op      = clone->Ist.WrTmp.data->Iex.Binop.op;
   IRExpr* arg1 = clone->Ist.WrTmp.data->Iex.Binop.arg1;
...
static void dt_smt2_binop_tt_01 ( IRStmt *clone )
{
   UInt ltmp    = clone->Ist.WrTmp.tmp;
   UInt op      = clone->Ist.WrTmp.data->Iex.Binop.op;
   IRExpr* arg1 = clone->Ist.WrTmp.data->Iex.Binop.arg1;
...
static void dt_smt2_binop_tt_11 ( IRStmt *clone )
{
   UInt ltmp    = clone->Ist.WrTmp.tmp;
   UInt op      = clone->Ist.WrTmp.data->Iex.Binop.op;
   IRExpr* arg1 = clone->Ist.WrTmp.data->Iex.Binop.arg1;
...
// ltmp = <op> rtmp1 rtmp2
void dt_smt2_binop_tt ( IRStmt *clone )
{
   IRExpr* arg1 = clone->Ist.WrTmp.data->Iex.Binop.arg1;
   IRExpr* arg2 = clone->Ist.WrTmp.data->Iex.Binop.arg2;
   UInt rtmp1   = arg1->Iex.RdTmp.tmp;
   ...
   if ( is_tainted(rtmp1) && !is_tainted(rtmp2) )
      dt_smt2_binop_tt_10(clone);
   else if ( !is_tainted(rtmp1) && is_tainted(rtmp2) )
      dt_smt2_binop_tt_01(clone);
   else if ( is_tainted(rtmp1) && is_tainted(rtmp2) )
      dt_smt2_binop_tt_11(clone);
}


// ltmp = rtmp
void dt_smt2_rdtmp ( IRStmt *clone )
{
...
// reg = tmp
void dt_smt2_put32_t ( IRStmt *clone )
{
...
// reg = tmp
void dt_smt2_put64_t ( IRStmt *clone )
{
...
void dt_smt2_x86g_calculate_condition ( IRStmt *clone )
{
   UInt ltmp    = clone->Ist.WrTmp.tmp;
   IRExpr *data = clone->Ist.WrTmp.data;
   ...
   //VG_(printf)("(assert (= t%d_%d (%s)))\n", ltmp, _ti(ltmp),
   //         dt_smt2_amd64g_calculate_condition_cond( clone, buf ) );
...
static char *dt_smt2_amd64g_calculate_condition_op_tc_common( IRStmt *clone, char *buf, UInt dep1tmp, ULong dep2c )
{
   IRExpr *data = clone->Ist.WrTmp.data;
...
static char *dt_smt2_amd64g_calculate_condition_op_tc( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   ...
   return dt_smt2_amd64g_calculate_condition_op_tc_common( clone, buf, dep1tmp, dep2c );
}

static char *dt_smt2_amd64g_calculate_condition_op_ct_common( IRStmt *clone, char *buf, ULong dep1c, UInt dep2tmp )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   IRExpr *op   = data->Iex.CCall.args[1];
   ULong cc_op;
...
static char *dt_smt2_amd64g_calculate_condition_op_ct( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   IRExpr *dep1 = data->Iex.CCall.args[2];
   IRExpr *dep2 = data->Iex.CCall.args[3];
   ...
   return dt_smt2_amd64g_calculate_condition_op_ct_common( clone, buf, dep1c, dep2tmp );
}

static char *dt_smt2_amd64g_calculate_condition_op_tt_01( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   IRExpr *dep1 = data->Iex.CCall.args[2];
   IRExpr *dep2 = data->Iex.CCall.args[3];
...
static char *dt_smt2_amd64g_calculate_condition_op_tt_10( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   IRExpr *dep1 = data->Iex.CCall.args[2];
   IRExpr *dep2 = data->Iex.CCall.args[3];
...
static char *dt_smt2_amd64g_calculate_condition_op_tt_11( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   IRExpr *dep1 = data->Iex.CCall.args[2];
   IRExpr *dep2 = data->Iex.CCall.args[3];
...
static char *dt_smt2_amd64g_calculate_condition_op( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   IRExpr *dep1 = data->Iex.CCall.args[2];
   IRExpr *dep2 = data->Iex.CCall.args[3];
   ...
   if ( dep1->tag == Iex_RdTmp && dep2->tag == Iex_Const )
      return dt_smt2_amd64g_calculate_condition_op_tc(clone, buf);
   else if ( dep1->tag == Iex_Const && dep2->tag == Iex_RdTmp )
      return dt_smt2_amd64g_calculate_condition_op_ct(clone, buf);
   else if ( dep1->tag == Iex_RdTmp && dep2->tag == Iex_RdTmp ) {
      UInt dep1tmp = dep1->Iex.RdTmp.tmp;
      UInt dep2tmp = dep2->Iex.RdTmp.tmp;
      if ( !is_tainted(dep1tmp) && is_tainted(dep2tmp) )
         return dt_smt2_amd64g_calculate_condition_op_tt_01(clone, buf);
      else if ( is_tainted(dep1tmp) && !is_tainted(dep2tmp) )
         return dt_smt2_amd64g_calculate_condition_op_tt_10(clone, buf);
      else if ( is_tainted(dep1tmp) && is_tainted(dep2tmp) )
         return dt_smt2_amd64g_calculate_condition_op_tt_11(clone, buf);
      else
         tl_assert(0);
   } else
      tl_assert(0);
...
static char *dt_smt2_amd64g_calculate_condition_cond( IRStmt *clone, char *buf )
{
   IRExpr *data = clone->Ist.WrTmp.data;
   ...
   switch ( cond )
   {
      case AMD64CondZ:
         VG_(sprintf)(tmp, "ite (= %s) #x%016llx #x%016llx",
            dt_smt2_amd64g_calculate_condition_op(clone, buf),
            1LL, 0LL );
         break;
      case AMD64CondNZ:
...
void dt_smt2_amd64g_calculate_condition ( IRStmt *clone )
{
   UInt ltmp    = clone->Ist.WrTmp.tmp;
   IRExpr *data = clone->Ist.WrTmp.data;
   UInt ty      = data->Iex.CCall.retty - Ity_INVALID;
...
void dt_smt2_ite_tt ( IRStmt *clone )
{
   UInt ltmp    = clone->Ist.WrTmp.tmp;
   IRExpr *data = clone->Ist.WrTmp.data;
