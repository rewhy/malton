...
VgHashTable *dt_malloc_list  = NULL;   // HP_Chunks
...
void* record_block( ThreadId tid, void* p, SizeT req_szB, SizeT slop_szB )
{
   ...
   hc->data     = (Addr)p;
   VG_(HT_add_node)(dt_malloc_list, hc);

   // Untaint malloc'd block
   dt_make_mem_untainted( (Addr)p, hc->req_szB + hc->slop_szB ); 
...
void unrecord_block ( void* p )
{
   ...
   HP_Chunk* hc = VG_(HT_remove)(dt_malloc_list, (UWord)p);
   if (NULL == hc) {
      return;   // must have been a bogus free()
   }

   // Untaint freed block
   dt_make_mem_noaccess( (Addr)p, hc->req_szB + hc->slop_szB ); 
...
void* realloc_block ( ThreadId tid, void* p_old, SizeT new_req_szB )
{
   ...
   hc = VG_(HT_remove)(dt_malloc_list, (UWord)p_old);
   if (hc == NULL) {
   ...
   if (new_req_szB <= old_req_szB + old_slop_szB) {
      ...
      // Copy taint state
      dt_copy_address_range_state( (Addr)p_old, (Addr)p_new, new_actual_szB );
   }
...
void* dt_malloc ( ThreadId tid, SizeT szB )
{
...
void* dt___builtin_new ( ThreadId tid, SizeT szB )
{
	void* res = alloc_and_record_block( tid, szB, VG_(clo_alignment), /*is_zeroed*/True );
#if 0
	if ( dt_clo_taint_begin )
...
void* dt___builtin_vec_new ( ThreadId tid, SizeT szB )
{
	void* res = alloc_and_record_block( tid, szB, VG_(clo_alignment), /*is_zeroed*/True );
	return res;
}

void* dt_calloc ( ThreadId tid, SizeT m, SizeT szB )
{
	void* res = alloc_and_record_block( tid, m*szB, VG_(clo_alignment), /*is_zeroed*/True );
	return res;
}

void* dt_memalign ( ThreadId tid, SizeT alignB, SizeT szB )
{
	void* res = alloc_and_record_block( tid, szB, alignB, True );
	return res;
}

void dt_free ( ThreadId tid __attribute__((unused)), void* p )
{
   unrecord_block(p);
   VG_(cli_free)(p);
}
...
void* dt_realloc ( ThreadId tid, void* p_old, SizeT new_szB )
{
...
{                                                            
   HP_Chunk* hc = VG_(HT_lookup)( dt_malloc_list, (UWord)p );
