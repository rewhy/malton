...
static void init_auxmap_L1_L2 ( void )
{
	...
	// Anyway, it creates an AVL tree here, the node element seems to be an `AuxMapEnt`
...
static INLINE SecMap** get_secmap_low_ptr ( Addr a )
{
	// a: aa bb cc dd
	// a >> 16: 00 00 aa bb
	UWord pm_off = a >> 16;  // So here we have the "entry" of the "primary map"
	// Warning: `16` should be `sizeof(Word) * 8 - N_PRIMARY_BITS`
	// On 32-bit CPU, `32 - 16 = 16`
	tl_assert(pm_off < N_PRIMARY_MAP);
	return &primary_map[ pm_off ];  // So this is the "SecMap" of the 
}
...
{
	return ( a <= MAX_PRIMARY_ADDRESS  // Highly suspected to be forever true
...
struct {
	Addr  a;
	UChar vbits8[BYTES_PER_SEC_VBIT_NODE];  // A `vbits8` represents 4 byte, so 16 `vbits8`s represents 64 bytes (why?)
}
SecVBitNode;
...
ULong dt_LOADVn_slow ( Addr a, SizeT nBits, Bool bigendian )
{
	...
	//= /*dt_clo_partial_loads_ok*/ 0 && szB == VG_WORDSIZE
	//                             && VG_IS_WORD_ALIGNED(a)
	//                             && n_addrs_bad < VG_WORDSIZE;

	//   if (n_addrs_bad > 0 && !partial_load_exemption_applies)
	//      dt_record_address_error( VG_(get_running_tid)(), a, szB, False );
...
void dt_STOREVn_slow ( Addr a, SizeT nBits, ULong vbytes, Bool bigendian )
{
	...
	//   if (n_addrs_bad > 0)
	//      dt_record_address_error( VG_(get_running_tid)(), a, szB, True );
}
...
Bool dt_check_mem_tainted( Addr a, SizeT len ) {
	// This function is suspected to be a helper function that checks
	// if a given base address and the size of the memory area
	// contains tainted bytes
	Bool isT = False, ok;
	// Since `isT` is returned to the caller, the full name of this
	// variable gotta be `isTainted`
	// The variable `ok` turns to be a flag that marks if the given
	// address is valid
	SizeT i;
	UChar vbits8;
	// Some logging stuff
#ifdef DBG_TAINT_SET
	if( dt_clo_taint_begin )
		TNT_LOGI("dt_check_mem_tainted: a_0x%x l_%d\n", a, len);
#endif
	for(i = 0; i < len; i++) {
		// Iterate and check the memory area byte by byte?
		ok = get_vbits8(a + i, &vbits8);
		// Anyway, now we have `vbits8` that represents current byte
		if (vbits8 == V_BITS8_UNTAINTED)
			continue;  // If current byte is untainted, check next byte
		// Else, we found a god damn tainted byte, so assign `True` to this flag
		isT = True;
...
#else
		// If we get here, then we must have found a tainted byte,
		// and the whole area of memory is considered "tainted",
		// so no need to check the remaining bytes, except for
		// debugging purpose
		break;
#endif
		// You get here because the `DBG_TAINT_SET` macro is defined
		// (or you will have broke the iteration)
		if( ok == False )
			DT_LOGI("0x%x is no addressable.\n", a+i);
	}
	// Tell the caller whether this area of memory is tainted
	return isT;
}
...
void dt_make_mem_tainted_named ( Addr a, SizeT len, const HChar *varname ) {
	...
	// smt2
#ifdef DBG_TAINT_SET
	DT_LOGI("%s\n","dt_make_mem_tainted a_0x%x, l_%d\n", a, len);
	if ( dt_clo_smt2_format) {
		UInt i = 0;
		for ( ; i < len; i++) {
...
void dt_make_mem_untainted( Addr beg, SizeT len ) {
	PROF_EVENT(42, "dt_make_mem_untainted");
	set_address_range_perms(beg, len, VA_BITS16_UNTAINTED, SM_DIST_UNTAINTED );
#ifdef DBG_TAINT_SET
	DT_LOGI("%s\n","dt_make_mem_untainted a_0x%x, l_%d\n", beg, len);
#endif
}
...
void dt_copy_address_range_state(Addr src, Addr dst, SizeT len) {
	...
	PROF_EVENT(50, "dt_copy_address_range_state");
	...
	if (nooverlap && aligned) {
		...
		if (src < dst) {
			for (i = 0, j = len-1; i < len; i++, j--) {
				PROF_EVENT(51, "dt_copy_address_range_state(loop)");
		...
		if (src > dst) {
			for (i = 0; i < len; i++) {
				PROF_EVENT(52, "dt_copy_address_range_state(loop)");
				vabits2 = get_vabits2( src+i );
				set_vabits2( dst+i, vabits2 );
				if (VA_BITS2_PARTUNTAINTED == vabits2) {
					set_sec_vbits8( dst+i, get_sec_vbits8( src+i ) );
	...
	if( dt_clo_taint_begin ) {
		...
		dt_check_mem_tainted(src, ol);
		dt_check_mem_tainted(dst, ol);
	}
#endif
}
...
VG_REGPARM(2) void dt_helperc_LOADV256be ( /*OUT*/V256* res, Addr a )
{
	dt_LOADV_128_or_256(&res->w64[0], a, 256, True);
#ifdef DBG_LOAD
	VG_(printf)("dt_helperc_LOADV256be 0x%08lx\n");
#endif
}
VG_REGPARM(2) void dt_helperc_LOADV256le ( /*OUT*/V256* res, Addr a )
{
	dt_LOADV_128_or_256(&res->w64[0], a, 256, False);
#ifdef DBG_LOAD
	VG_(printf)("dt_helperc_LOADV256le 0x%08lx\n");
#endif
}

VG_REGPARM(2) void dt_helperc_LOADV128be ( /*OUT*/V128* res, Addr a )
{
	dt_LOADV_128_or_256(&res->w64[0], a, 128, True);
#ifdef DBG_LOAD
	VG_(printf)("dt_helperc_LOADV128be 0x%08lx\n");
#endif
}
VG_REGPARM(2) void dt_helperc_LOADV128le ( /*OUT*/V128* res, Addr a )
{
	dt_LOADV_128_or_256(&res->w64[0], a, 128, False);
#ifdef DBG_LOAD
	VG_(printf)("dt_helperc_LOADV128le 0x%08lx\n");
#endif
}
/* ------------------------ Size = 8 ------------------------ */
...
VG_REGPARM(1) ULong dt_helperc_LOADV64be ( Addr a )
{
	ULong result = dt_LOADV64(a, True);
#ifdef DBG_LOAD
	VG_(printf)("dt_helperc_LOADV64be 0x%08lx 0x%llx\n", a, result);
#endif
	return result;
}
VG_REGPARM(1) ULong dt_helperc_LOADV64le ( Addr a )
{
	ULong result = dt_LOADV64(a, False);
#ifdef DBG_LOAD
	VG_(printf)("dt_helperc_LOADV64le 0x%08lx 0x%llx\n", a, result);
#endif
	return result;
}
...
VG_REGPARM(1) void dt_helperc_STOREV64be ( Addr a, ULong vbits64 )
{
	dt_STOREV64(a, vbits64, True);
#ifdef DBG_STORE
	VG_(printf)("dt_helperc_STOREV64be 0x%08lx 0x%llx\n", a, vbits64);
#endif
}
VG_REGPARM(1) void dt_helperc_STOREV64le ( Addr a, ULong vbits64 )
{
	dt_STOREV64(a, vbits64, False);
#ifdef DBG_STORE
	VG_(printf)("dt_helperc_STOREV64le 0x%08lx 0x%llx\n", a, vbits64);
#endif
}
...
VG_REGPARM(1) UWord dt_helperc_LOADV32be ( Addr a )
{
	UWord result = dt_LOADV32(a, True);
#ifdef DBG_LOAD
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_LOADV32be 0x%08lx 0x%lx\n", a, result);
#endif
	return result;
}

VG_REGPARM(1) UWord dt_helperc_LOADV32le ( Addr a )
{
	UWord result = dt_LOADV32(a, False);
#ifdef DBG_LOAD
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_LOADV32le 0x%08lx 0x%lx\n", a, result);
#endif
	return result;
}
...
VG_REGPARM(2) void dt_helperc_STOREV32be ( Addr a, UWord vbits32 )
{
#ifdef DBG_STORE
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_STOREV32be 0x%08lx 0x%lx\n", a, vbits32);
#endif
...
VG_REGPARM(2) void dt_helperc_STOREV32le ( Addr a, UWord vbits32 )
{
#ifdef DBG_STORE
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_STOREV32le 0x%08lx 0x%lx\n", a, vbits32);
#endif
...
VG_REGPARM(1) UWord dt_helperc_LOADV16be ( Addr a )
{
	UWord result = dt_LOADV16(a, True);
#ifdef DBG_LOAD
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_LOADV16be 0x%08lx 0x%lx\n", a, result);
#endif
	return result;
}
VG_REGPARM(1) UWord dt_helperc_LOADV16le ( Addr a )
{
	UWord result = dt_LOADV16(a, False);
#ifdef DBG_LOAD
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_LOADV16le 0x%08lx 0x%lx\n", a, result);
#endif
	return result;
}
...
VG_REGPARM(2) void dt_helperc_STOREV16be ( Addr a, UWord vbits16 )
{
#ifdef DBG_STORE
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_STOREV16be 0x%08lx 0x%lx\n", a, vbits16);
#endif

	dt_STOREV16(a, vbits16, True);
}
VG_REGPARM(2) void dt_helperc_STOREV16le ( Addr a, UWord vbits16 )
{
#ifdef DBG_STORE
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_STOREV16le 0x%08lx 0x%lx\n", a, vbits16);
#endif
...
VG_REGPARM(1) UWord dt_helperc_LOADV8 ( Addr a )
{
...
#ifdef DBG_LOAD
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_LOADV8 0x%08lx 0x%x\n", a, result);
#endif
	return result;
#else
	{
...
VG_REGPARM(2) void dt_helperc_STOREV8 ( Addr a, UWord vbits8 )
{
#ifdef DBG_STORE
	if( dt_clo_taint_begin )
		VG_(printf)("dt_helperc_STOREV8 0x%08lx 0x%lx\n", a, vbits8);
#endif
...
// These arrays are initialised to 0 in dt_clo_post_init
...
dt_describe_data(address, varname, 1024, &type, &var_loc); 


// dt_check_var_access(tid, varname, VAR_WRITE, type, var_loc);

// if(!dt_do_print)  dt_do_print = 1;
// if(!dt_do_print && taint)  dt_do_print = 1;
//	if(!dt_do_print && dt_clo_taint_begin )  dt_do_print = 1;
//	if(!taint) return; 
#define H_EXIT_EARLY \
	if( dt_clo_tainted_ins_only && !taint) return;
...
#define H_EXIT_EARLY_LDST \
	if( dt_clo_tainted_ins_only && !(taint | is_tainted(atmp))) return;
...
// if <gtmp> goto <jk> dst
VG_REGPARM(3) void dt_h32_exit_t (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {

		if( dt_clo_critical_ins_only ) return;
		H_EXIT_EARLY;
		H_SMT2(smt2_exit);
		H32_PC;
...
VG_REGPARM(3) void dt_h32_exit_c (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// JMP tmp
VG_REGPARM(3)	void dt_h32_next_t (
			IRExpr *clone, 
			UInt value, 
			UInt taint ) {
...
// JMP const 
VG_REGPARM(3)	void dt_h32_next_c (
			IRExpr *clone, 
			UInt value, 
			UInt taint ) {
...
// STORE <end> atmp = dtmp
VG_REGPARM(3)	void dt_h32_store_tt (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
		if( dt_clo_critical_ins_only ) return;
...
// STORE atmp = const
VG_REGPARM(3)	void dt_h32_store_tc (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
// STORE const = dtmp
VG_REGPARM(3)	void dt_h32_store_ct (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
// ltmp = LOAD <ty> atmp
VG_REGPARM(3)	void dt_h32_load_t (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// ltmp = LOAD <ty> c
VG_REGPARM(3)	void dt_h32_load_c (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_get_i (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// reg = tmp
VG_REGPARM(3)	void dt_h32_put_t (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
		// Reg book-keeping
...
// reg = const 
VG_REGPARM(3)	void dt_h32_put_c (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
		// Reg bookkeeping
...
VG_REGPARM(3)	void dt_h32_put_i (
			UInt tt1, 
			UInt tt2, 
			UInt value, 
			UInt taint ) {
...
// ltmp = <op> const 
VG_REGPARM(3)	void dt_h32_wrtmp_c (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// ltmp = <op> rtmp
VG_REGPARM(3)	void dt_h32_unop_t (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// ltmp = <op> const
VG_REGPARM(3)	void dt_h32_unop_c (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// ltmp = <op> rtmp1, const
VG_REGPARM(3)	void dt_h32_binop_tc (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
// ltmp = <op> Const rtmp1
VG_REGPARM(3)	void dt_h32_binop_ct (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
// ltmp = <op> rtmp1 rtmp2
VG_REGPARM(3)	void dt_h32_binop_tt (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_binop_cc (
			IRStmt *clone,
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_triop (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// ltmp = rtmp
VG_REGPARM(3)	void dt_h32_rdtmp (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_ite_tc (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_ite_ct (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_ite_tt (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h32_ite_cc (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
// ltmp = callee( arg[0], ... )
VG_REGPARM(3)	void dt_h32_ccall (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
//        );
VG_REGPARM(3)	void dt_h32_x86g_calculate_condition (
			IRStmt *clone, 
			UInt value, 
			UInt taint ) {
...
VG_REGPARM(3)	void dt_h64_exit_t (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// IF <gtmp> GOTO <jk> addr
VG_REGPARM(3)	void dt_h64_exit_c (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// JMP tmp
VG_REGPARM(3)	void dt_h64_next_t (
			IRExpr *clone, 
			ULong value, 
			ULong taint ) {
...
// JMP const 
VG_REGPARM(3)	void dt_h64_next_c (
			IRExpr *clone, 
			ULong value, 
			ULong taint ) {
...
// STORE atmp = dtmp
VG_REGPARM(3)	void dt_h64_store_tt (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// STORE atmp = c
VG_REGPARM(3)	void dt_h64_store_tc (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// STORE c = dtmp
VG_REGPARM(3)	void dt_h64_store_ct (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = LOAD <ty> atmp
VG_REGPARM(3)	void dt_h64_load_t (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = LOAD <ty> const
VG_REGPARM(3)	void dt_h64_load_c (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
VG_REGPARM(3)	void dt_h64_get_i (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// reg = tmp
VG_REGPARM(3)	void dt_h64_put_t (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
		// Reg book-keeping
...
// reg = const
VG_REGPARM(3)	void dt_h64_put_c (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
		// Reg bookkeeping
...
VG_REGPARM(3)	void dt_h64_put_i (
			ULong tt1, 
			ULong tt2, 
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> ...
VG_REGPARM(3)	void dt_h64_wrtmp_c (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> rtmp
VG_REGPARM(3)	void dt_h64_unop_t (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> const
VG_REGPARM(3)	void dt_h64_unop_c (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> rtmp1 const
VG_REGPARM(3)	void dt_h64_binop_tc (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> const rtmp2
VG_REGPARM(3)	void dt_h64_binop_ct (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> rtmp1 rtmp2
VG_REGPARM(3)	void dt_h64_binop_tt (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> const1 const2
VG_REGPARM(3)	void dt_h64_binop_cc (
			IRStmt *clone,
			ULong value, 
			ULong taint ) {
...
// ltmp = <op> rtmp1, rtmp2, rtmp3
VG_REGPARM(3)	void dt_h64_triop (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
VG_REGPARM(3)	void dt_h64_rdtmp (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = ctmp? rtmp1 : const
VG_REGPARM(3)	void dt_h64_ite_tc (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = ctmp? const : rtmp2
VG_REGPARM(3)	void dt_h64_ite_ct (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = ctmp? rtmp1 : rtmp2
VG_REGPARM(3)	void dt_h64_ite_tt (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = ctmp? const1 : const2
VG_REGPARM(3)	void dt_h64_ite_cc (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
// ltmp = callee( arg[0], ... )
VG_REGPARM(3)	void dt_h64_ccall (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
//        );
VG_REGPARM(3)	void dt_h64_amd64g_calculate_condition (
			IRStmt *clone, 
			ULong value, 
			ULong taint ) {
...
void dt_describe_data(Addr addr, HChar* varnamebuf, UInt bufsize, enum VariableType* type, enum VariableLocation* loc) {
...
void init_shadow_memory(void) {
	...
	init_auxmap_L1_L2();  // What the hell is this?

	/* auxmap_size = auxmap_used = 0;
		 no ... these are statically initialised */

	/* Secondary V bit table */
	secVBitTable = createSecVBitTable();  // So it's actually "T bit" now (Taint bit)
	// And this table should be a table that expands the "partially tainted" status

#if 0
	// Taintgrind: Solely for testing
	dt_make_mem_tainted(0xbe000000, 0x1000000);
#endif
}

#if 0
void dt_check_var_access(ThreadId tid, const HChar* varname, Int var_request, enum VariableType type, enum VariableLocation var_loc) {
