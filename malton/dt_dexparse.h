#ifndef _DT_DEX_PARSE_H
#define _DT_DEX_PARSE_H
...
struct DexCode {
	...
	UInt	  insnsSize;          /* size of the insns array, in UShort units */
	UShort  insns[1];
	/* followed by optional UShort padding */
...
Bool oatDexParse(Addr, UInt, Addr, UInt);
...
struct DexOrJar {
	...
	//JarFile*    pJarFile;
	void*			  pJarFile;
	UChar*         pDexMemory; // malloc()ed memory, if any
};
...
struct RegisterMap {
	/* header */
	UChar      format;         /* enum RegisterMapFormat; MUST be first entry */
	UChar      regWidth;       /* bytes per register line, 1+ */
	UChar      numEntries[2];  /* number of entries */
...
struct Method {
	...
	/* access flags; low 16 bits are defined by spec (could be UShort?) */
	UInt		          accessFlags;
	UShort			        methodIndex;

	UShort              registersSize;  /* ins + locals */
	UShort              outsSize;
	UShort              insSize;
	const char*     name;
	struct DexProto prototype;
	const char*     shorty;
	const UShort*       insns;          /* instructions, in memory-mapped .dex */
	int             jniArgInfo;
...
struct Thread {
	...
	const UInt*         jitResumeNSP;     // Native SP at return point
	const UShort*         jitResumeDPC;     // Dalvik inst following single-step
	JitState    jitState;
	int         icRechainCount;
	...
	const UShort*   currTraceHead;  // Start of the trace we're building
	const UShort*   currRunHead;    // Start of run we're building
	int         currRunLen;     // Length of run in 16-bit words
	const UShort*   lastPC;         // Stage the PC for the threaded interpreter
	...
	UInt spillRegion[MAX_SPILL_JIT_IA];
#endif
#endif
};
...
extern const HChar* ACCESS_FLAG_NAMES[20];
extern const UInt		ACCESS_FLAG_VALUES[20];
...
typedef struct TaintSourceDesc {
	HChar *clazz;
	HChar *method;
	UChar tag;
} TaintSourceDesc;
...
enum {
	TAG_NONE       = 0x00,
	TAG_DEVICE     = 0x01,  // getDeviceId
	TAG_DATABASE   = 0x02,  // Contacts, ...
	TAG_LOCATION   = 0x04,  // Location.*
	TAG_SENSOR     = 0x08,  // Gyro?
	TAG_USAGE      = 0x10,  // Data usage
	TAG_FILESYSTEM = 0x20,
	TAG_MEDIA      = 0x40,
	TAG_OTHER			 = 0x80
};
...
Bool oatDexParse(Addr oatdata, UInt oatdata_size, Addr oatexec, UInt oatexec_size);
