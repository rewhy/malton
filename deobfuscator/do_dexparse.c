...
Bool dumpRawData(UChar* buf, UInt size, Addr a) {
	...
	Int fout;
	HChar fpath[255];
	VG_(sprintf)(fpath, "/data/local/tmp/fuzz/0x%08x-%d-%d-%d.dex", (Addr)buf, size, file_index++, a);
	fout = VG_(fd_open)(fpath, VKI_O_WRONLY|VKI_O_TRUNC, 0);
	if (fout <= 0) {
		fout = VG_(fd_open)(fpath, VKI_O_CREAT|VKI_O_WRONLY, VKI_S_IRUSR|VKI_S_IWUSR);
		if( fout <= 0 ) {
			VG_(printf)("Create Dex file error.\n");
			return;
		}
	} 
	VG_(printf)("Try to dump data into file %s 0x%08x-0x%08x\n", 
...
void dumpOatMem(UChar* buf, UInt size) {
	dumpRawData(buf, size, 0);
}
#endif

/*
 * Add a newly found Dex file into the Dex file list
 */
...
/*
 * Release the found Dex file list and save the data into sdcard
 */
void releaseDexFileList() {
	...
	while(tfl) {
		...
		if(pfl->cloneMem)
		{
			...
			// Try to reconstruct the Dex file
			processDexFile(pfl->pDexFileClone);
			// Try to write the raw data of the constructed Dex files into sdcard
			dumpRawData((Addr)pHeader, fileLen, (Addr)pfl->pDexFile);
			// Free the memory storing the Dex data
...
/*
 * Check whether a Dex file is already found
 */
 ...
 * Dump a "code" struct.
 */
void dumpCode(const struct DexCode *pCode)
{
 ...
 */
static Bool dumpFileHeader(struct DexFile* pDexFile, struct MonitorDexFile* pMDexFile, Bool isProcess)
{
	struct DexOptHeader* pOptHeader = pDexFile->pOptHeader;
	struct DexHeader* pHeader = pDexFile->pHeader;
	...
	if ((pHeader != NULL) && (pHeader == pDexFile->baseAddr)) {
		...
		if(isProcess) {
			if(VG_(memcmp)(pHeader->magic, "dex\n035\0", 8) != 0) {
				VG_(memcpy)(pHeader->magic, "dex\n035\0", 8);
			}
		}
		...
		OAT_LOGI("\tsignature           : %02x%02x...%02x%02x\n",	pHeader->signature[0], pHeader->signature[1],
				pHeader->signature[kSHA1DigestLen-2],	pHeader->signature[kSHA1DigestLen-1]);
		OAT_LOGI("\tfile_size           : %d (0x%08x)\n", pHeader->fileSize, pHeader->fileSize);
		OAT_LOGI("\theader_size         : %d\n", pHeader->headerSize);
		OAT_LOGI("\tlink_size           : %d\n", pHeader->linkSize);
		if(isProcess) {
			if ((pHeader->linkOff == 0) && (pDexFile->pLinkData > pDexFile->baseAddr)) {
				pHeader->linkOff = (Addr)pDexFile->pLinkData - (Addr)pDexFile->baseAddr;
			}
		}
		OAT_LOGI("\tlink_off            : %d (0x%06x)\n",	pHeader->linkOff, pHeader->linkOff);
		OAT_LOGI("\tstring_ids_size     : %d\n", pHeader->stringIdsSize);
		if(isProcess) {
			if ((pHeader->stringIdsOff == 0) && (pDexFile->pStringIds > pDexFile->baseAddr)) {
				pHeader->stringIdsOff = (Addr)pDexFile->pStringIds - (Addr)pDexFile->baseAddr;
			}
		}
		OAT_LOGI("\tstring_ids_off      : %d (0x%06x)\n",	pHeader->stringIdsOff, pHeader->stringIdsOff);
		OAT_LOGI("\ttype_ids_size       : %d\n", pHeader->typeIdsSize);
		if(isProcess) {
			if ((pHeader->typeIdsOff == 0) && (pDexFile->pTypeIds > pDexFile->baseAddr)) {
				pHeader->typeIdsOff = (Addr)pDexFile->pTypeIds - (Addr)pDexFile->baseAddr;
			}
		}
		OAT_LOGI("\ttype_ids_off        : %d (0x%06x)\n",	pHeader->typeIdsOff, pHeader->typeIdsOff);
		OAT_LOGI("\tproto_ids_size      : %d\n", pHeader->protoIdsSize);
		if(isProcess) {
			if ((pHeader->protoIdsOff == 0) && (pDexFile->pProtoIds > pDexFile->baseAddr)) {
				pHeader->protoIdsOff = (Addr)pDexFile->pProtoIds - (Addr)pDexFile->baseAddr;
			}
		}
		OAT_LOGI("\tproto_ids_off       : %d (0x%06x)\n", pHeader->protoIdsOff, pHeader->protoIdsOff);
		OAT_LOGI("\tfield_ids_size      : %d\n", pHeader->fieldIdsSize);
		if(isProcess) {
			if ((pHeader->fieldIdsOff == 0) && (pDexFile->pFieldIds > pDexFile->baseAddr)) {
				pHeader->fieldIdsOff = (Addr)pDexFile->pFieldIds - (Addr)pDexFile->baseAddr;
			}
		}
		OAT_LOGI("\tfield_ids_off       : %d (0x%06x)\n",	pHeader->fieldIdsOff, pHeader->fieldIdsOff);
		OAT_LOGI("\tmethod_ids_size     : %d\n", pHeader->methodIdsSize);
		if(isProcess) {
			if ((pHeader->methodIdsOff == 0) && (pDexFile->pMethodIds > pDexFile->baseAddr)) {
				pHeader->methodIdsOff = (Addr)pDexFile->pMethodIds - (Addr)pDexFile->baseAddr;
			}
		}
		OAT_LOGI("\tmethod_ids_off      : %d (0x%06x)\n", pHeader->methodIdsOff, pHeader->methodIdsOff);
		OAT_LOGI("\tclass_defs_size     : %d\n", pHeader->classDefsSize);
		if(isProcess) {
			if ((pHeader->classDefsOff == 0) && (pDexFile->pClassDefs > pDexFile->baseAddr)) {
				pHeader->classDefsOff = (Addr)pDexFile->pClassDefs - (Addr)pDexFile->baseAddr;
			}
		}
...
{
	if( !pDexFile ) {
		OAT_LOGD("Error DexFile\n");
		return;
	}
	dumpRegisterMaps(pDexFile, NULL); // 2.1
	dumpFileHeader(pDexFile, NULL, False);		// 2.2
...
struct MonitorDexFile* createDexFileMem(const struct DexFile* pDexFile, Addr addr, UInt len) {
	...
	dumpFileHeader(pDexFileClone, NULL, True);
	return pMDexFile;
#else
	return NULL;
#endif
...
static	Bool copyMethod(struct MonitorDexFile* pMDexFile, 
	...
	OAT_LOGI("Try to copy DexCode off 0x%0x (0x%0x) of DexFile 0x%08x\n", pDexMethod->codeOff, 
...
Bool copyDexClass(const struct DexFile* pDexFile, Int idx, HChar* desc) {
	...
	OAT_LOGI("Try to copy interface in class %d of DexFile 0x%08x\n", idx, (Addr)pDexFile);
	...
	/* Clone Class Data */
	OAT_LOGI("Try to copy ClassData in class %d of DexFile 0x%0x\n", idx, (Addr)pDexFile);
	...
	OAT_LOGI("Origi baseAddr 0x%08x classDataOff 0x%08x\n", (Addr)pDexFile->baseAddr, pClassDef->classDataOff); 
	OAT_LOGI("Clone baseAddr 0x%08x classDataOff 0x%08x\n", (Addr)pDexFileClone->baseAddr, pClassDef1->classDataOff);

	const UChar*								pEncodeData1= dexGetClassData(pDexFileClone, pClassDef1);
	OAT_LOGI("Try to copy Encodedata from 0x%08x to 0x%08x len=%d basaddr 0x%08x\n", 
...
/*
 * A new Dex file is found, then we create a new clone memory to store its data
 */
...
Bool copyOneClass(const struct DexFile* pDexFile, HChar* desc) {
	...
	for(i = 0; i < (Int)pDexFile->pHeader->classDefsSize; i++) {
		if(copyDexClass(pDexFile, i, desc) == True) {
			OAT_LOGI("Copy one class %s of DexFile 0x%08x successfully.\n", desc, (Addr)pDexFile);
			return True;
		}
	}
...
static Bool getMethodCode(struct DexFile *pDexFile, struct DexMethod* pDexMethod, struct Method *mths, Int count)
{
	...
	const struct DexCode* pDexCode = (struct DexCode*)(pDexFile->baseAddr + pDexMethod->codeOff);
	if(pDexCode->insnsSize > 0) {
		OAT_LOGI("Copy insn of method %s(%s) from 0x%08x to 0x%08x size=%d\n", name, shorty,
...
Bool getClassMethods(const struct DexFile *pDexFile, struct ClassObject *pClazz) {
	...
	OAT_LOGI("Try to get method code of class %s in DexFile 0x%08x\n", 
	...
	if( i == (Int)pDexFileClone->pHeader->classDefsSize ) {
		...
		OAT_LOGI("Found class %s with id %d in DexFile 0x%08x.\n", 
...
struct DexFile *DexFilePlus2DexFile(struct DexFilePlus *pDexFilePlus) {
...
/*
 * A new Dex found is found, and we initialize the structure to store its information and 
 * malloc memory to store its Dex data
 */
