...
extern DebugInfo *di_libart;
extern Addr libart_text_addr;
extern UInt libart_text_size;


extern Addr base_oatdata_addr;
extern UInt base_oatdata_size;
extern Addr base_oatexec_addr;
extern UInt base_oatexec_size;
extern Addr boot_oatdata_addr;
extern UInt boot_oatdata_size;
extern Addr boot_oatexec_addr;
extern UInt boot_oatexec_size;
...
Addr dexFileObjs[MAX_DEXFILE_NUM] = {0};
...
void saveDexFileObjs() {
	struct DexFilePlus *pDexFileObj = NULL;
	VG_(printf)("Try to save all Dex file objects");
	for( UInt i = 0; i < MAX_DEXFILE_NUM; i++ ) {
		if(dexFileObjs[i] != 0) {
			pDexFileObj = dexFileObjs[i];
			if( pDexFileObj->begin_ > 0 && pDexFileObj->size_ > 0 ) {
				VG_(printf)("Try to save Dex file object 0x%08x mem:0x%08x - 0x%08x\n", 
						(Addr)pDexFileObj, pDexFileObj->begin_, pDexFileObj->begin_ + pDexFileObj->size_);
				dumpRawData(pDexFileObj->begin_, pDexFileObj->size_, 3);
...
Bool addDexFileObj(Addr addr, Bool isSave) {
	...
	for( UInt i = 0; i < MAX_DEXFILE_NUM; i++ ) {
		if(dexFileObjs[i] == 0) {
			if( pDexFileObj->begin_ > 0 && pDexFileObj->size_ > 0 ) {
				...
				dexFileObjs[i] = addr;
				if(isSave) {
					dumpRawData(pDexFileObj->begin_, pDexFileObj->size_, 4);
					//DexMemParse(pDexFileObj->begin_, pDexFileObj->size_);
				}
				return True;
			}
...
#if 0
			VG_(printf)("Try to added repeat Dex file object 0x%08x mem:0x%08x - 0x%08x\n", 
					addr, pDexFileObj->begin_, pDexFileObj->begin_ + pDexFileObj->size_);
