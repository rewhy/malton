	...
	/*MOD*/XArray* maps /* XArray<DebugInfoMapping> */ )
{
	...
	for (i = 0; i < n; i++) {
		...
		TRACE_SYMTAB("  [%ld]    avma 0x%-16lx    size 0x%-8lx    "
				"foff 0x%-8llx    %s %s %s\n",
	...
	/*MOD*/XArray* maps /* XArray<DebugInfoMapping> */ )
{
	...
	for (i = 0; i < n; i++) {
		...
		for (j = i+1; j < n; j++) {
			...
			if (map_j->size == 0)
				continue;
			Bool overlap = overlaps_DebugInfoMappings( map_i, map_j );
...
ULong VG_(di_notify_mmap)( Addr a, Bool allow_SkFileV, Int use_fd )
{
	...
	if ( ! (seg->kind == SkFileC || (seg->kind == SkFileV && allow_SkFileV)) ) {
		if(debug)
			VG_(printf)("ignore non-file mappings.\n");
		return 0;
	}
	...
	if (!filename) {
		if(debug)
			VG_(printf)("the file doesn't have a name.\n");
		return 0;
	}
	...
	if (VG_(strncmp)(filename, "/proc/xen/", 10) == 0) {
		if(debug)
			VG_(printf)("reading debug info from /proc/xen/privcmd.\n");
		return 0;
	}
...
*/
	...
	UInt tmp_len = VG_(strlen)(filename); // Added by Rewhy
...
#  elif defined(VGA_amd64) || defined(VGA_ppc64be) || defined(VGA_ppc64le)  \
	...
	/* Added by Rewhy */
	if( tmp_len > 3 && filename[tmp_len-3]=='o' && filename[tmp_len-2]=='a' && filename[tmp_len-1]=='t') {
		is_ro_map = seg->hasR && !seg->hasW && !seg->hasX;
	} else {
		if( tmp_len > 4 && filename[tmp_len-4]=='o' && filename[tmp_len-3]=='d' && filename[tmp_len-2]=='e' && filename[tmp_len-1]=='x') {
			is_ro_map = seg->hasR && !seg->hasW && !seg->hasX;
		}
	}
	/* End Rewhy */
...
#  endif
	...
	if( tmp_len > 3 && filename[tmp_len-3]=='o' && filename[tmp_len-2]=='a' && filename[tmp_len-1]=='t') {
		di->isOat = True;
			// VG_(printf)(filename);
			VG_(printf)("Found OAT file: %s\n", filename);
	} else {
		if( tmp_len > 4 && filename[tmp_len-4] == 'o' && filename[tmp_len-3]=='d' && filename[tmp_len-2]=='e' && filename[tmp_len-1]=='x') {
			di->isOat = True;
			VG_(printf)("Found ODEX file: %s\n", filename);
			//return 0;
		}
	}
	/* End Rewhy */
	...
	if (debug) {
		VG_(printf)("di_notify_mmap-4: noting details in DebugInfo* at %p\n", di);
	}
	...
	if (di->fsm.have_rx_map && di->fsm.have_rw_map && !di->have_dinfo) {
		/* Ok, so, finally, we found what we need, and we haven't
			 ...
			 Yee-ha! */
		if (debug) {
			VG_(printf)("di_notify_mmap-5: achieved accept state for %s\n", filename);
		}
...
#  endif
	if (do_nothing /* wrong platform */) {
		if (debug)
			VG_(printf)("di_notify_vm_protect-2: wrong platform, doing nothing.\n");
		return;
	}
...
Bool VG_(get_rx_mapping_index) (/*IN*/ const DebugInfo* di, /*IN*/ const Int i, /*OUT*/Addr *addr, /*OUT*/Int *size)
{
	if( i < VG_(sizeXA)(di->fsm.maps) ) {
...
/* Added by Rewhy */
void VG_(print_sym_table)(const DebugInfo *si)
{
	UInt i = 0;
	VG_(printf)("print_sym_table: considering %s\n", si->soname);
	for (i = 0; i < si->symtab_used; i++) {
		VG_(printf)("symtab: 0x%08x %s\n", si->symtab[i].avmas.main, si->symtab[i].pri_name);
		const HChar** sec_names = si->symtab[i].sec_names;
		if(sec_names) {
			while(*sec_names) {
				VG_(printf)("secname: 0x%08x %s\n", si->symtab[i].avmas.main, *sec_names);
				sec_names++;
			}
		}
	}
}
/* for get memory range of a symbole according to its name
*/
Bool VG_(get_symbol_range_SLOW)(const DebugInfo *si, const HChar* name,
		Addr* avma,  SizeT* size)
{
	UInt i = 0;
	VG_(printf)("get_symbol_range_SLOW: considering %s\n", si->soname);
	for (i = 0; i < si->symtab_used; i++) {
		const HChar* pri_name = si->symtab[i].pri_name;
		vg_assert(pri_name);
		if (0==VG_(strcmp)(name, pri_name) ) {
			*avma = si->symtab[i].avmas.main;
			*size = si->symtab[i].size;
			return True;
		}
		const HChar** sec_names = si->symtab[i].sec_names;
		if (sec_names) {
...
Bool VG_(lookup_symbol)(/*IN*/const DebugInfo* si, /*IN*/const Addr addr, /*OUT*/HChar** name)
{
	vg_assert(si);
	*name = NULL;
	for (Int i = 0; i < si->symtab_used; i++) {
		const HChar* pri_name = si->symtab[i].pri_name;
		vg_assert(pri_name);
		if( addr == si->symtab[i].avmas.main ) {
			*name = pri_name;
			return True;
		}
	}
...
/* Added by Rewhy */
Bool VG_(DebugInfo_is_oat) (const DebugInfo *di)
{
	return di->isOat;
}
/* End Rewhy */
